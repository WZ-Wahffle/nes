...
void ppu_addr(uint8_t value) {
    static uint16_t prev_one = 1 << 12;
    static uint16_t prev_two = 1 << 12;
    static uint16_t prev_three = 1 << 12;
    if (!this->w) {
        this->t &= 0xff;
        this->t |= value << 8;
        this->t &= 0x3fff;
    ...
    prev_two = prev_one;
    prev_one = this->t;
    if ((prev_one & (1 << 12)) == 0 && (prev_two & (1 << 12)) == 0 &&
        (prev_three & (1 << 12)) == 0 && (this->t & (1 << 12)) != 0)
        this->a12_end_of_scanline_callback();
}
...
void oam_data_write(uint8_t value) {
    switch (this->oam_addr % 4) {
    case 0:
        this->oam[this->oam_addr / 4].y_pos = value + (value == 0xff ? 0 : 1);
        break;
    case 1:
        this->oam[this->oam_addr / 4].tile_index = value;
        break;
    case 2:
        this->oam[this->oam_addr / 4].attributes = value;
        break;
    case 3:
        this->oam[this->oam_addr / 4].x_pos = value;
        break;
    }
    this->oam_addr++;
}
...
void oam_dma(cpu_mmu *mem, uint8_t value) {
    for (uint8_t i = 0; i < 0x40; i++) {
        ...
        if(this->oam[i].y_pos != 0xff) this->oam[i].y_pos++;
...
void *ui_thread(void *_) {
    ...
    while (!WindowShouldClose()) {
        ...
        this->joy1_mirror =
            ...
            (IsKeyPressed(KEY_LEFT_SHIFT) << 2) |
            (IsGamepadButtonDown(joy1_idx, GAMEPAD_BUTTON_MIDDLE_RIGHT) << 3) |
            (IsKeyPressed(KEY_RIGHT_SHIFT) << 3) |
        ...
        for (uint16_t y = 0; y < VIEWPORT_HEIGHT; y++) {
            ...
            get_cpu_handle()->remaining_cycles += CYCLES_PER_LINE;
            if (this->a12_end_of_scanline_callback != NULL)
                this->a12_end_of_scanline_callback();
        }
        this->vblank = true;
